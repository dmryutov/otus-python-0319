...
static PyObject* py_deviceapps_xread_pb(PyObject* self, PyObject* args) {
    ...
    // Process data
    PyObject* output_generator = PyList_New(0);
    ...
    while (!gzeof(input_file)) {
        ...
        if (bytes > 0) {
            ...
            if (dict != NULL) {
                PyList_Append(output_generator, dict);
                Py_DECREF(dict);
            }
            else {
    ...
    return PySeqIter_New(output_generator);
}
...
static PyModuleDef PBModule = {
    ...
    "pb", // Name of module
    "Protobuf (de)serializer", // Module documentation, may be NULL
